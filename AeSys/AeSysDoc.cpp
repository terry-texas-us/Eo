...
BEGIN_MESSAGE_MAP(AeSysDoc, CDocument)
		ON_COMMAND(ID_PURGE_UNREFERENCEDBLOCKS, &AeSysDoc::OnPurgeUnreferencedBlocks)
		ON_COMMAND(ID_CLEAR_ACTIVELAYERS, &AeSysDoc::OnClearActiveLayers)
		ON_COMMAND(ID_CLEAR_ALLLAYERS, &AeSysDoc::OnClearAllLayers)
		ON_COMMAND(ID_CLEAR_ALLTRACINGS, &AeSysDoc::OnClearAllTracings)
		ON_COMMAND(ID_CLEAR_MAPPEDTRACINGS, &AeSysDoc::OnClearMappedTracings)
		ON_COMMAND(ID_CLEAR_VIEWEDTRACINGS, &AeSysDoc::OnClearViewedTracings)
		ON_COMMAND(ID_CLEAR_WORKINGLAYER, &AeSysDoc::OnClearWorkingLayer)
		ON_COMMAND(ID_EDIT_IMAGETOCLIPBOARD, &AeSysDoc::OnEditImageToClipboard)
		ON_COMMAND(ID_EDIT_SEGTOWORK, &AeSysDoc::OnEditSegToWork)
		ON_COMMAND(ID_EDIT_TRACE, &AeSysDoc::OnEditTrace)
		ON_COMMAND(ID_EDIT_TRAPCOPY, &AeSysDoc::OnEditTrapCopy)
		ON_COMMAND(ID_EDIT_TRAPCUT, &AeSysDoc::OnEditTrapCut)
		ON_COMMAND(ID_EDIT_TRAPDELETE, &AeSysDoc::OnEditTrapDelete)
		ON_COMMAND(ID_EDIT_TRAPPASTE, &AeSysDoc::OnEditTrapPaste)
		ON_COMMAND(ID_EDIT_TRAPQUIT, &AeSysDoc::OnEditTrapQuit)
		ON_COMMAND(ID_EDIT_TRAPWORK, &AeSysDoc::OnEditTrapWork)
		ON_COMMAND(ID_EDIT_TRAPWORKANDACTIVE, &AeSysDoc::OnEditTrapWorkAndActive)
		ON_COMMAND(ID_FILE, &AeSysDoc::OnFile)
		ON_COMMAND(ID_FILE_QUERY, &AeSysDoc::OnFileQuery)
		ON_COMMAND(ID_FILE_MANAGE, &AeSysDoc::OnFileManage)
		ON_COMMAND(ID_FILE_SEND_MAIL, &CDocument::OnFileSendMail)
		ON_COMMAND(ID_FILE_TRACING, &AeSysDoc::OnFileTracing)
#ifdef OD_OLE_SUPPORT
		ON_COMMAND(ID_INSERT_OLEOBJECT, &AeSysDoc::OnInsertOleobject)
#endif // OD_OLE_SUPPORT
		ON_COMMAND(ID_LAYER_ACTIVE, &AeSysDoc::OnLayerActive)
		ON_COMMAND(ID_LAYER_LOCK, &AeSysDoc::OnLayerLock)
		ON_COMMAND(ID_LAYER_OFF, &AeSysDoc::OnLayerOff)
		ON_COMMAND(ID_LAYER_MELT, &AeSysDoc::OnLayerMelt)
		ON_COMMAND(ID_LAYER_CURRENT, &AeSysDoc::OnLayerCurrent)
		ON_COMMAND(ID_LAYERS_SETALLACTIVE, &AeSysDoc::OnLayersSetAllActive)
		ON_COMMAND(ID_LAYERS_SETALLLOCKED, &AeSysDoc::OnLayersSetAllLocked)
		ON_COMMAND(ID_PURGE_UNUSEDLAYERS, &AeSysDoc::OnPurgeUnusedLayers)
		ON_COMMAND(ID_PURGE_DUPLICATEOBJECTS, &AeSysDoc::OnPurgeDuplicateObjects)
		ON_COMMAND(ID_PURGE_EMPTYNOTES, &AeSysDoc::OnPurgeEmptyNotes)
		ON_COMMAND(ID_PURGE_EMPTYGROUPS, &AeSysDoc::OnPurgeEmptyGroups)
		ON_COMMAND(ID_PENS_REMOVEUNUSEDSTYLES, &AeSysDoc::OnPensRemoveUnusedLinetypes)
		ON_COMMAND(ID_PENS_EDITCOLORS, &AeSysDoc::OnPensEditColors)
		ON_COMMAND(ID_PENS_LOADCOLORS, &AeSysDoc::OnPensLoadColors)
		ON_COMMAND(ID_PENS_TRANSLATE, &AeSysDoc::OnPensTranslate)
		ON_COMMAND(ID_PRIM_BREAK, &AeSysDoc::OnPrimBreak)
		ON_COMMAND(ID_PRIM_EXTRACTNUM, &AeSysDoc::OnPrimExtractNum)
		ON_COMMAND(ID_PRIM_EXTRACTSTR, &AeSysDoc::OnPrimExtractStr)
		ON_COMMAND(ID_TOOLS_PRIMITIVE_SNAPTOENDPOINT, &AeSysDoc::OnToolsPrimitiveSnapToEndPoint)
		ON_COMMAND(ID_PRIM_GOTOCENTERPOINT, &AeSysDoc::OnPrimGotoCenterPoint)
		ON_COMMAND(ID_TOOLS_PRIMITVE_DELETE, &AeSysDoc::OnToolsPrimitiveDelete)
		ON_COMMAND(ID_PRIM_MODIFY_ATTRIBUTES, &AeSysDoc::OnPrimModifyAttributes)
		ON_COMMAND(ID_TOOLS_GROUP_BREAK, &AeSysDoc::OnToolsGroupBreak)
		ON_COMMAND(ID_TOOLS_GROUP_DELETE, &AeSysDoc::OnToolsGroupDelete)
		ON_COMMAND(ID_TOOLS_GROUP_DELETELAST, &AeSysDoc::OnToolsGroupDeleteLast)
		ON_COMMAND(ID_TOOLS_GROUP_EXCHANGE, &AeSysDoc::OnToolsGroupExchange)
		ON_COMMAND(ID_TOOLS_GROUP_RESTORE, &AeSysDoc::OnToolsGroupRestore)
		ON_COMMAND(ID_SETUP_PENCOLOR, &AeSysDoc::OnSetupPenColor)
		ON_COMMAND(ID_SETUP_LINETYPE, &AeSysDoc::OnSetupLinetype)
		ON_COMMAND(ID_SETUP_FILL_HOLLOW, &AeSysDoc::OnSetupFillHollow)
		ON_COMMAND(ID_SETUP_FILL_SOLID, &AeSysDoc::OnSetupFillSolid)
		ON_COMMAND(ID_SETUP_FILL_PATTERN, &AeSysDoc::OnSetupFillPattern)
		ON_COMMAND(ID_SETUP_FILL_HATCH, &AeSysDoc::OnSetupFillHatch)
		ON_COMMAND(ID_SETUP_NOTE, &AeSysDoc::OnSetupNote)
		ON_COMMAND(ID_SETUP_SAVEPOINT, &AeSysDoc::OnSetupSavePoint)
		ON_COMMAND(ID_SETUP_GOTOPOINT, &AeSysDoc::OnSetupGotoPoint)
		ON_COMMAND(ID_SETUP_OPTIONS_DRAW, &AeSysDoc::OnSetupOptionsDraw)
		ON_COMMAND(ID_TRACING_ACTIVE, &AeSysDoc::OnTracingActive)
		ON_COMMAND(ID_TRACING_LOCK, &AeSysDoc::OnTracingLock)
		ON_COMMAND(ID_TRACING_OFF, &AeSysDoc::OnTracingOff)
		ON_COMMAND(ID_TRACING_FUSE, &AeSysDoc::OnTracingFuse)
		ON_COMMAND(ID_TRACING_CURRENT, &AeSysDoc::OnTracingCurrent)
		ON_COMMAND(ID_TRAPCOMMANDS_COMPRESS, &AeSysDoc::OnTrapCommandsCompress)
		ON_COMMAND(ID_TRAPCOMMANDS_EXPAND, &AeSysDoc::OnTrapCommandsExpand)
		ON_COMMAND(ID_TRAPCOMMANDS_INVERT, &AeSysDoc::OnTrapCommandsInvert)
		ON_COMMAND(ID_TRAPCOMMANDS_SQUARE, &AeSysDoc::OnTrapCommandsSquare)
		ON_COMMAND(ID_TRAPCOMMANDS_QUERY, &AeSysDoc::OnTrapCommandsQuery)
		ON_COMMAND(ID_TRAPCOMMANDS_FILTER, &AeSysDoc::OnTrapCommandsFilter)
		ON_COMMAND(ID_TRAPCOMMANDS_BLOCK, &AeSysDoc::OnTrapCommandsBlock)
		ON_COMMAND(ID_TRAPCOMMANDS_UNBLOCK, &AeSysDoc::OnTrapCommandsUnblock)
		ON_COMMAND(ID_HELP_KEY, &AeSysDoc::OnHelpKey)
		...
		ON_COMMAND(ID_EDIT_UNDO, &AeSysDoc::OnEditUndo)
		ON_UPDATE_COMMAND_UI(ID_EDIT_UNDO, &AeSysDoc::OnUpdateEditUndo)
		ON_UPDATE_COMMAND_UI(ID_EDIT_REDO, &AeSysDoc::OnUpdateEditRedo)
		ON_COMMAND(ID_EDIT_REDO, &AeSysDoc::OnEditRedo)
...
BOOL AeSysDoc::DoSave(const wchar_t* pathName, const BOOL replace) {
	...
	if (PathName.IsEmpty()) { // Save As
		...
		if (DoPromptFileName(PathName, static_cast<unsigned>(replace != 0 ? AFX_IDS_SAVEFILE : AFX_IDS_SAVEFILECOPY), OFN_HIDEREADONLY | OFN_EXPLORER | OFN_PATHMUSTEXIST, FALSE, DocTemplate) == 0) {
			return FALSE;
		} // don't even attempt to save 
	}
	if (OnSaveDocument(PathName) == 0) {
		if (pathName == nullptr) {
			TRY
				{
...
void AeSysDoc::OnUpdateVectorizerType(CCmdUI* commandUserInterface) {
	commandUserInterface->Enable(static_cast<BOOL>(m_Viewer == nullptr && !theApp.RecentGsDevicePath().isEmpty()));
}
...
OdGePoint3d AeSysDoc::getPoint(const OdString& prompt, const int options, OdEdPointTracker* tracker) {
	if (m_Macro.get() != nullptr && !m_Macro->isEof()) {
...
void AeSysDoc::OnEditConsole() {
	...
	OdSaveState<bool> SavedConsoleMode(m_Console, true);
	try {
		if (m_Viewer != nullptr && m_Viewer->isGettingString()) {
...
void AeSysDoc::ExecuteCommand(const OdString& command, const bool echo) {
	OdSaveState<int> SavedCommandActive(m_CommandActive);
	++m_CommandActive;
	auto CommandContext(CommandContext0());
...
BOOL AeSysDoc::OnCmdMsg(const unsigned commandId, const int messageCategory, void* commandObject, AFX_CMDHANDLERINFO* handlerInfo) {
	if (handlerInfo == nullptr) {
		...
		if (TopMenu != nullptr) { // Check if it is theApp's dynamic menu item
			...
			if (TopMenu->GetMenuItemInfoW(commandId, &MenuItemInfo, FALSE) != 0) {
				...
				if (MenuItemInfo.dwItemData == gsl::narrow_cast<unsigned>(theApp.GetGsMenuItemMarker())) {
					...
					if (messageCategory == CN_COMMAND) {
						...
						static_cast<CCmdUI*>(commandObject)->Enable(static_cast<BOOL>(m_Viewer == nullptr));
						static_cast<CCmdUI*>(commandObject)->SetCheck(static_cast<int>(Vectorizer == static_cast<const wchar_t*>(theApp.RecentGsDevicePath())));
					}
					return TRUE;
				}
				// </tas>
...
BOOL AeSysDoc::OnOpenDocument(const wchar_t* file) {
	...
	switch (FileType) {
		...
		case EoDb::kPeg: {
			...
			if (PegFile.Open(file, CFile::modeRead | CFile::shareDenyNone, &FileException) != 0) {
				PegFile.Load(this);
				saveAsType_ = EoDb::kPeg;
			}
			break;
...
BOOL AeSysDoc::OnSaveDocument(const wchar_t* pathName) {
	...
	switch (saveAsType_) {
		case EoDb::kPeg: {
			...
			CFileException FileException;
			if (DwgToPegFile.Open(pathName, CFile::modeCreate | CFile::modeWrite | CFile::shareExclusive, &FileException) != 0) {
...
EoDbGroup* AeSysDoc::DeletedGroupsRemoveHead() {
	...
	if (m_DeletedGroupList.IsEmpty() == 0) {
...
void AeSysDoc::RemoveEmptyLayers() {
	for (auto LayerIndex = GetLayerTableSize() - 1; LayerIndex > 0; LayerIndex--) {
		auto Layer {GetLayerAt(LayerIndex)};
		if (Layer != nullptr && Layer->IsEmpty() != 0) {
			Layer->DeleteGroupsAndRemoveAll();
			delete Layer;
			m_LayerTable.RemoveAt(LayerIndex);
		}
...
void AeSysDoc::WriteShadowFile() {
	if (saveAsType_ == EoDb::kPeg) {
		...
		if (Extension > 0) {
			...
			CFileException FileException;
			EoDbPegFile PegFile(m_DatabasePtr);
			if (PegFile.Open(ShadowFilePath, CFile::modeWrite, &FileException) == 0) {
...
void AeSysDoc::OnPrimBreak() {
	...
	if (Group != nullptr && ActiveView->EngagedPrimitive() != nullptr) {
		...
		if (Primitive->IsKindOf(RUNTIME_CLASS(EoDbPolyline)) != 0) {
			...
			EoDbBlock* Block;
			if (LookupBlock(BlockReference->Name(), Block)) {
				Group->FindAndRemovePrimitive(Primitive);
...
void AeSysDoc::OnFileQuery() {
	...
	if (Layer != nullptr) {
		...
		if (MenuResource == IDR_LAYER) {
			SubMenu->CheckMenuItem(ID_LAYER_CURRENT, static_cast<unsigned>(MF_BYCOMMAND | static_cast<long>(Layer->IsCurrent()) ? MF_CHECKED : MF_UNCHECKED));
			SubMenu->CheckMenuItem(ID_LAYER_ACTIVE, static_cast<unsigned>(MF_BYCOMMAND | static_cast<long>(Layer->IsActive()) ? MF_CHECKED : MF_UNCHECKED));
			SubMenu->CheckMenuItem(ID_LAYER_LOCK, static_cast<unsigned>(MF_BYCOMMAND | static_cast<long>(Layer->IsLocked()) ? MF_CHECKED : MF_UNCHECKED));
			SubMenu->CheckMenuItem(ID_LAYER_OFF, static_cast<unsigned>(MF_BYCOMMAND | static_cast<long>(Layer->IsOff()) ? MF_CHECKED : MF_UNCHECKED));
		} else {
			SubMenu->CheckMenuItem(ID_TRACING_CURRENT, static_cast<unsigned>(MF_BYCOMMAND | static_cast<long>(Layer->IsCurrent()) ? MF_CHECKED : MF_UNCHECKED));
			SubMenu->CheckMenuItem(ID_TRACING_ACTIVE, static_cast<unsigned>(MF_BYCOMMAND | static_cast<long>(Layer->IsActive()) ? MF_CHECKED : MF_UNCHECKED));
			SubMenu->CheckMenuItem(ID_TRACING_LOCK, static_cast<unsigned>(MF_BYCOMMAND | static_cast<long>(Layer->IsLocked()) ? MF_CHECKED : MF_UNCHECKED));
			SubMenu->CheckMenuItem(ID_TRACING_OFF, static_cast<unsigned>(MF_BYCOMMAND | static_cast<long>(Layer->IsOff()) ? MF_CHECKED : MF_UNCHECKED));
		}
...
void AeSysDoc::OnPensTranslate() {
	...
	if (StreamFile.Open(AeSys::ResourceFolderPath() + L"\\Pens\\xlate.txt", CFile::modeRead | CFile::typeText) != 0) {
		wchar_t Line[128];
...
void AeSysDoc::OnPrimExtractNum() {
	auto ActiveView {AeSysView::GetActiveView()};
	const auto CurrentPnt {ActiveView->GetCursorPosition()};
	if (ActiveView->SelectGroupAndPrimitive(CurrentPnt) != nullptr) {
		const auto Primitive {ActiveView->EngagedPrimitive()};
		CString Number;
...
void AeSysDoc::OnDrawingUtilitiesAudit() {
	...
	if (theApp.auditDialog->Create(IDD_AUDITINFO) == 0) {
