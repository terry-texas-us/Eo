...
EoDbEllipse::EoDbEllipse() 
	: m_Center(OdGePoint3d::kOrigin), m_MajorAxis(OdGeVector3d::kXAxis), m_MinorAxis(OdGeVector3d::kYAxis), m_SweepAngle(TWOPI) {
}
...
void EoDbEllipse::AssociateWith(OdDbBlockTableRecordPtr blockTableRecord) {
	...
	if (!PlaneNormal.isZeroLength())
	{
		...
		double RadiusRatio = m_MinorAxis.length() / m_MajorAxis.length();
...
void EoDbEllipse::CutAt(const OdGePoint3d& point, EoDbGroup* group, OdDbDatabasePtr database) {
	...
	double Relationship = SwpAngToPt(point) / m_SweepAngle;

	if (Relationship <= DBL_EPSILON || Relationship >= 1. - DBL_EPSILON) {
		return;
	}
	double SweepAngle = m_SweepAngle * Relationship;
...
void EoDbEllipse::FormatGeometry(CString& geometry) const {
	OdGeVector3d Normal = m_MajorAxis.crossProduct(m_MinorAxis);
...
int EoDbEllipse::IsWithinArea(const OdGePoint3d& lowerLeftCorner, const OdGePoint3d& upperRightCorner, OdGePoint3d* intersections) {
	...
	double dRad = OdGeVector3d(ptBeg - m_Center).length();
	...
	for (int i1 = 0; i1 < iInts; i1++) { // Sort intersections from begin to end of sweep
		for (int i2 = 1; i2 < iInts - i1; i2++) {
			if (fabs(dIntAng[i2]) < fabs(dIntAng[i2 - 1])) {
				...
				dIntAng[i2] = dAng;
				OdGePoint3d pt = intersections[i2 - 1];
...
EoDbEllipse* EoDbEllipse::ConstructFrom(EoDbFile& file) {
	...
	OdGePoint3d CenterPoint(file.ReadPoint3d());
	OdGeVector3d MajorAxis(file.ReadVector3d());
	OdGeVector3d MinorAxis(file.ReadVector3d());
...
EoDbEllipse* EoDbEllipse::Create(const EoDbEllipse& other, OdDbDatabasePtr database) {
	OdDbBlockTableRecordPtr BlockTableRecord = database->getModelSpaceId().safeOpenObject(OdDb::kForWrite);
	// <Teigha>Possibly need additional typing of the ObjecId producted by cloning</Teigha>
...
bool pFndCPGivRadAnd4Pts(double radius, OdGePoint3d arLn1Beg, OdGePoint3d arLn1End, OdGePoint3d arLn2Beg, OdGePoint3d arLn2End, OdGePoint3d* center) {
	...
	OdGeVector3d vPlnNorm;

	OdGeVector3d v1 = OdGeVector3d(arLn1End - arLn1Beg); // Determine vector defined by endpoints of first line
	dV1Mag = v1.length();
	...
	vPlnNorm = v1.crossProduct(v2); // Determine vector normal to tangent vectors
	...
	EoGeMatrix3d tm;
	tm.setToWorldToPlane(OdGePlane(arLn1Beg, vPlnNorm));

	arLn1End.transformBy(tm);
	arLn2Beg.transformBy(tm);
	arLn2End.transformBy(tm);
...
int pFndSwpAngGivPlnAnd3Lns(const OdGeVector3d& planeNormal, const OdGePoint3d& arP1, const OdGePoint3d& arP2, const OdGePoint3d& arP3, const OdGePoint3d& center, double& sweepAngle) {
	...
	// None of the points coincide with center point
	EoGeMatrix3d tm;
	tm.setToWorldToPlane(OdGePlane(center, planeNormal));
